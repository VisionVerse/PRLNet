...
class EuclideanAngleLossWithOHEM(nn.Module):
    ...
    def __cal_weight(self, gt):
        ...
        labels = torch.unique(gt, sorted=True)[1:]  # 类别数 [1, 2, 3]
        ...
        if posCount != 0:
            ...
            for segi in labels:
                ...
                weight = weight * (~overlap_segi).to(torch.float) + \
                         pixAve * overlap_segi.to(torch.float)
        # weight = weight[None]
        # weight.shape [H, W]
        return weight

    def forward(self, df_out, gt_df, gt, weight=None):
        """
        df_out, pred: (N, C, H, W)
        gt_df: (N, C, H, W)
        gt: (N, 1, H, W)
        """
        # L1 and L2 distance
        N, _, H, W = df_out.shape

        distL1 = df_out - gt_df
        distL2 = distL1**2

        theta_p = cart2polar(df_out)
        theta_g = cart2polar(gt_df)
        angleDistL1 = theta_g - theta_p

        if weight is None:
            weight = torch.zeros((N, H, W), device=df_out.device)
            for i in range(N):
        ...
        weightPos = torch.zeros_like(gt, dtype=df_out.dtype)
        weightNeg = torch.zeros_like(gt, dtype=df_out.dtype)
        ...
        # total loss
        a = torch.sum(((distL2[:, 0, ...] + distL2[:, 1, ...]) + angleDistL1**2) *
                      (weightPos + weightNeg)
        )
        total_loss = a / N / 2.0 / torch.sum(weightPos + weightNeg)
