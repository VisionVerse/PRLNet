...
def get_saliency_smoothness(pred, gt, df):
    alpha = 10
    s1 = torch.norm(df, dim=1, keepdim=True) + 10  # dim=1 表示在第二维上计算模长
    s2 = 0
    ...
    # smooth_loss = torch.mean(s1 * cps_xy) + s2 * torch.mean(weighted_lap)
    smooth_loss = torch.mean(s1) * torch.mean(cps_xy) + s2 * torch.mean(weighted_lap)
